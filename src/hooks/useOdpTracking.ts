 ...
 *
 * Calls window.zaius.event() directly (SDK loaded in index.html).
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          ...
          odpEvent('vswriter_navigation', { section: id });
    ...
    function handleClick(e: MouseEvent) {
      ...
      if (btn) {
        ...
        odpEvent('vswriter_click', { element: 'cta', label, href });
        return;
      }
      ...
      if (faqTrigger) {
        ...
        odpEvent('vswriter_click', { element: 'faq', title });
      }
    }
