...
/** Run a Graph query and return the first CompetitorComparisonPage item or null */
async function queryGraph(authKey: string, query: string, variables: Record<string, unknown>) {
  ...
  return json?.data?.CompetitorComparisonPage?.items?.[0] ?? null
}
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (url.pathname === '/api/content') {
          ...
          try {
            const item = await queryGraph(authKey, PAGE_QUERY, { slug: `/${slug}/` })
        ...
        if (url.pathname === '/api/preview') {
          ...
          try {
            const item = await queryGraph(singleKey, PREVIEW_QUERY, variables)
