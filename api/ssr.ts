...
function buildHeadHtml(page: any): string {
  ...
  const webPageLd: Record<string, unknown> = {
    ...
    url: canonicalHref,
    publisher: { '@type': 'Organization', name: 'Optimizely', url: 'https://www.optimizely.com' },
  ...
  const resolved = (page.Testimonials ?? []).filter((t: any) => t.item?.Quote && t.item?.AuthorName);
...
// ---- HTML Template ----
// dist/index.html is included via vercel.json "includeFiles" and read at runtime.
...
function getHtmlTemplate(): string {
  ...
  try {
    ...
    // Fallback shell — assets won't load but the page won't hard-crash
    _htmlTemplate = '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body><div id="root"></div></body></html>';
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!page) {
      // Unknown slug — serve SPA shell so client renders the NotFound component
      res.setHeader('Content-Type', 'text/html');
    ...
    console.error('[api/ssr] handler failed:', err);
    res.setHeader('Content-Type', 'text/html');
    return res.status(200).send(htmlTemplate);
  }
}
